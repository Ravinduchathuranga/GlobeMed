...
import com.rc008code.hms.auth.decorators.service.LoggingAuthenticator;
...
//
    public static void testValidators() {
        ...
        // Chain them: first check not null, then check length
        // This order is more logical - check for null first, then check length
        notNullValidator.setNext(lengthValidator);

        // Test cases
        String[] testCases = {
            null,                          // Should fail: null (handled by NotNullValidator)
            "short",                      // Should fail: too short (handled by LengthValidator)
            "validpassword",              // Should pass
            "1234567890"                  // Should pass
        };

        // Run tests
        System.out.println("=== Testing Validator Chain (NotNull -> Length) ===");
        for (String test : testCases) {
            System.out.println("\nTesting: " + test);
            // Start the validation chain with the first validator
            ValidationResult result = notNullValidator.validate(test);
    ...
    public static void main(String[] args) {
        // Test the validators
        testValidators();
        Authenticator authenticator=new LoggingAuthenticator(new BasicAuthenticator());
        authenticator.authenticate("admin","admin");
        PasswordManager.hashPassword("password");
    }
}
        /* Uncomment the following code if you need to test other functionality
        try {
            // Initialize BOs
